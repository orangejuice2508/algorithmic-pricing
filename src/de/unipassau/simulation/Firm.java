...
class Firm {
    ...
    Firm(SimulationRun simulationRun, int seed) {
        ...
        qlearning = new QLearning(seed, competition);
    }
     ...
     *
     * @return action result (= price/quantity) that has been chosen by the Q-Learning algorithm.
     */
    int chooseAction() {
        if (simulationRun.getNumberOfConvergedPeriods() == SimulationManager.minNumberOfConvergedPeriods - 1 &&
                simulationRun.getNumberOfDeviations() < 1) {
            // Increment the number of  invoked deviations in order to prevent too many deviations.
            simulationRun.incrementNumberOfDeviations();

            // Set periods for detailed export.
            SimulationManager.firstPeriodForDetailedExport = simulationRun.getNumberOfPeriods() - 5;
            SimulationManager.lastPeriodForDetailedExport = simulationRun.getNumberOfPeriods() + 50;

            // Force deviation.
            return qlearning.deviation(getActionsOfOtherFirms());
        } else {
            // Run episode according to Q-Learning algorithm.
     ...
     */
    public List<Double> getPrices() { return competition.getPrices(); }

    /**
     * @return all prices of this firm as integers as stored in its corresponding competition instance.
     */
    public List<Integer> getPricesForDetailedExport() { return competition.getPricesForDetailedExport(); }

    /**
     * @return all quantities of this firm as integers as stored in its corresponding competition instance.
     */
    public List<Integer> getQuantitiesForDetailedExport() { return competition.getQuantitiesForDetailedExport(); }

    /**
     * @return all profits of this firm as integers as stored in its corresponding competition instance.
     */
    public List<Integer> getProfitsForDetailedExport() {
        return competition.getProfitsForDetailedExport();
    }
