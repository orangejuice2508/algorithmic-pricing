...
class PriceCompetition extends Competition {
    ...
    public double getScaledNashEquilibrium(int numberOfFirms) {
        return getPriceInNashEquilibrium(numberOfFirms);
    }

    /**
     * @param pricesOfOtherFirms prices of the other firms
     * @return the scaled best response price to the other firms' prices.
     */
    @Override
    public double getScaledBestResponse(Integer[] pricesOfOtherFirms) {
        // Calculate the "sum part" of the formula.
        double sum = 0;
        for (Integer price : pricesOfOtherFirms) {
            sum += omega - price;
        }
        ...
        return (omega / 2) - ((theta * sum) / (2 * (1 + theta * (numberOfFirms - 2))));
    }
